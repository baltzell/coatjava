...
public class AdaptiveTests {
	...
	public static void lineTest() {
		...
		try {
			adaptiveSwimmer.swimLine(Q, xo, yo, zo, p, theta, phi, targetLine, accuracy, maxPathLength,
					stepsizeAdaptive, eps, result);
...
//		num = 1;
		...
		InitialValues[] ivals = InitialValues.getInitialValues(rand, num, -1, false, 0., 0., 0., 0., 0., 0., 0.5, 5.0,
				10., 25., -30., 30.);
...
//					adaptiveSwimmer.swim(revIv.charge, revIv.xo, revIv.yo, revIv.zo, revIv.p, revIv.theta, revIv.phi, result.getFinalS(), stepsizeAdaptive, eps, result);
					adaptiveSwimmer.swimS(revIv.charge, revIv.xo, revIv.yo, revIv.zo, revIv.p, revIv.theta, revIv.phi,
							accuracy, result.getFinalS(), stepsizeAdaptive, eps, result);
					...
					// System.out.println("Bad swim to plane for i = " + i + " final pathlength = "
					// + result.getFinalS());
				}
			} // for
	...
	public static void TIMEzTest() {
		...
		// the initial values
		InitialValues[] ivals = InitialValues.getInitialValues(rand, num, 1, true, -0.05, -0.05, -0.05, // vertex mins
		...
		for (int i = n0; i < num; i++) {
			...
			try {
				traj = swimmer.swim(iv.charge, iv.xo, iv.yo, iv.zo, iv.p, iv.theta, iv.phi, zTarg, accuracy,
						maxPathLength, stepsizeAdaptive, Swimmer.CLAS_Tolerance, hdata);
		...
		System.out.println(String.format("[OLD] Adaptive time: %-7.3f", (time) / 1000.));
		...
		System.out.println(String.format("[NEW] Adaptive time: %-7.3f", (time) / 1000.));
	...
	public static void xxxxxxzTest() {
		...
		for (int i = n0; i < num; i++) {
			InitialValues.randomInitVal(rand, iv, 1, true, -0.05, -0.05, -0.05, // vertex mins
					0.05, 0.05, 0.05, // vertex max
					5, 8.0, // momentum range
					20., 35., // theta range
					0., 360. // phi range
...
//		num = 1;
		...
		InitialValues[] ivals = InitialValues.getInitialValues(rand, num, 1, true, 0., 0., 0., 0., 0., 0., 0.25, 1.0,
				40., 70., 0., 360.);
...
//		 adaptive step
		try {
			...
			System.out.println(String.format(
					"Adaptive time: %-7.3f   avg good delta = %-9.5f  max delta = %-9.5f  badStatCnt = %d",
					(time) / 1000., sum / (num - badStatusCount), delMax, badStatusCount));
			System.out
					.println("Adaptive Avg NS = " + (int) (((double) nStepTotal) / num) + "   MAX NS: " + nsMax + "\n");
		...
		try {
			...
			System.out.println(String.format(
					"NEW Adaptive time: %-7.3f   avg good delta = %-9.5f  max delta = %-9.5f  badStatCnt = %d",
					(time) / 1000., sum / (num - badStatusCount), delMax, badStatusCount));
			System.out.println(
					"NEW Adaptive Avg NS = " + (int) (((double) nStepTotal) / num) + "   MAX NS: " + nsMax + "\n");
	...
	public static void planeTest() {
		...
		Swimmer swimmer = new Swimmer(); // old swimmer
		AdaptiveSwimmer adaptiveSwimmer = new AdaptiveSwimmer();
		...
		try {
			...
			for (int i = n0; i < num; i++) {
				...
				try {
					swimmer.swimPlaneInterp(iv.charge, iv.xo, iv.yo, iv.zo, iv.p, iv.theta, iv.phi,
							plane, accuracy, maxPathLength, stepsizeAdaptive, eps, result1);
					
					uf1 = result1.getUf();
					status = result1.getStatus();
					if (status == AdaptiveSwimmer.SWIM_SUCCESS) {
						dist1 = plane.distance(uf1[0], uf1[1], uf1[2]);
						String s1 = String.format("OLD (%-10.6f, %-10.6f, %-10.6f) ", uf1[0], uf1[1], uf1[2]);
						String s2 = String.format("D: %-6.2e m", dist1);
						String s3 = result1.getIntersection().toString();

						System.out.print(s1 + s2 + s3);

					} else {
						System.out.println(
								"(OLD) Bad swim to plane for i = " + i + "  final pathlength = " + result1.getFinalS());
					}
				...
				// interp
				adaptiveSwimmer.swimPlaneInterp(iv.charge, iv.xo, iv.yo, iv.zo, iv.p, iv.theta, iv.phi, plane, accuracy,
						maxPathLength, stepsizeAdaptive, eps, result2);

				uf2 = result2.getUf();
				status = result2.getStatus();
				if (status == AdaptiveSwimmer.SWIM_SUCCESS) {
					dist2 = plane.distance(uf2[0], uf2[1], uf2[2]);
					String s1 = String.format("  NEW (%-10.6f, %-10.6f, %-10.6f) ", uf2[0], uf2[1], uf2[2]);
					...
					System.out.println(
							"(NEW) Bad swim to plane for i = " + i + "  final pathlength = " + result2.getFinalS());
				}
				System.out.println();
			}
	...
	public static void cylinderTest() {
		...
		try {
			...
			System.out
					.println(String.format("Rho time: %-7.3f   avg delta = %-9.5f  max delta = %-9.5f  badStatCnt = %d",
		...
		try {
			...
			System.out.println(
					String.format("Cylinder time: %-7.3f   avg delta = %-9.5f  max delta = %-9.5f  badStatCnt = %d",
							(time) / 1000., sum / num, delMax, badStatusCount));
			System.out
					.println("Cylinder Avg NS = " + (int) (((double) nStepTotal) / num) + "   MAX NS: " + nsMax + "\n");
